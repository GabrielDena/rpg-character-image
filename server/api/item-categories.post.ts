...
export default defineEventHandler(async (event) => {
    ...
    const db = useDb();
    const rows = await db.insert(itemCategories).values({ adventureId, name }).returning();
    const category = rows[0];
    if (!category) throw createError({ statusCode: 500, message: 'Failed to create category' });
